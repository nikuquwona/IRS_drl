...
with open('filename_48.txt', 'r') as f:
    data = np.array(eval(f.read()))
data = data[:,:3]
reduced_data = np.vstack([data[::25], data[-1]])
data = reduced_data 
...
# Save the animation
ani.save('3d_trajectory_hollow_points_reference_zlim_no_repeat_no_duplicate_legend6.mp4', writer='ffmpeg')
